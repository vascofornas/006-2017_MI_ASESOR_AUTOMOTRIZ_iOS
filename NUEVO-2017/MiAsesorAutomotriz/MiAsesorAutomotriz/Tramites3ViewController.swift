...
class Tramites3ViewController: UIViewController, UITextViewDelegate,MFMailComposeViewControllerDelegate,MFMessageComposeViewControllerDelegate {
    ...
    @IBAction func aceptarButton(_ sender: Any) {
        ...
        let agencia = UserDefaults.standard.object(forKey:"miCodigoAgencia") as? String
          let tramite = UserDefaults.standard.object(forKey:"miTramite") as? String
        ...
        let request = NSMutableURLRequest(url: NSURL(string: "http://www.miasesorautomotriz.com/php_ios/scripts/enviarTramites.php")! as URL)
        request.httpMethod = "POST"
        let postString = "a=\(nombre_usuario!)&b=\(email_usuario!)&c=\(tel_usuario!)&d=\(modelo!)&e=\(ano!)&f=\(km!)&h=\(fecha!)&i=\(hora!)&j=\(comentarios!)&k=\(codigo_cita)&l=\(agencia!)&n=\(emailAsesor!)&t=\(tramite!)"
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) {
            ...
            let alertController = UIAlertController(title: "Gracias", message: "Consulta de Trámites Online " + codigo_cita + " enviada con éxito", preferredStyle: .alert)
            let defaultAction = UIAlertAction(title: "Cerrar", style: .default, handler:  { action in self.performSegue(withIdentifier: "segue3", sender: self) })
